...
from visualisation import show_solution
...
class Profiler:
    ...
    def __call__(self, func):
        ...
        def function(self, *args, **kwargs):
            ...
            try:
                profiler_self.profile[func.__name__]["cum_time"] += time
            except KeyError:
                profiler_self.profile[func.__name__] = {"cum_time":time, "count":0}
            profiler_self.profile[func.__name__]["count"] += 1
    ...
    def display(self):
        print(f"Class: {self.cls_name}")
        try:
            max_function_length = max([len(name) for name in self.profile.keys()])
        except ValueError:
            print("No methods used")
            return
        profile_values = self.profile.values()
        counts = [x["count"] for x in profile_values]
        max_count = max([len(str(count)) for count in counts])

        print(f"\t" + "Function".ljust(max_function_length), end="\t")
        print(f"\t" + "Count".ljust(max_count), end="\t")
        print(f"\t" + "Run Time", end="\t")
        print(f"\t" + "Time/Run", end="\n")

        for function in self.profile:
            print(f"\t{function.ljust(max_function_length)}", end="\t")
            print(f"\t{str(self.profile[function]['count']).ljust(max_count)}", end="\t")
            print(f"\t{self.profile[function]['cum_time']:.6f}", end="\t")
            print(f"\t{self.profile[function]['cum_time']/self.profile[function]['count']:.8f}", end="\n")
...
class GreedySolver(SubSolver):
    ...
    profiler = Profiler("GreedySolver")
...
class GreedyMultiSolver(SubSolver):
    ...
    profiler = Profiler("GreedyMultiSolver")
...
class ProblemSplitter:
    ...
    profiler = Profiler("ProblemSplitter")
...
if __name__ == "__main__":
    ...
    solution = solve_iterable(target)
    solve_time = perf_counter() - start_time

    print(f"Total time taken: {solve_time:.3f}")

    classes = [Node, NodeHeader, DLXSolver, GreedySolver, Transformer, GreedyMultiSolver, ProblemSplitter]
    for class_ in classes:
        class_.profiler.display()
        print("\n\n")
